...
namespace AcidarX.AXImGui
{
    internal static class Util
    {
        ...
        public static void CreateVertexBuffer
            (this GL gl, string name, out uint buffer) => gl.CreateBuffer($"VBO: {name}", out buffer);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void CreateElementBuffer
            (this GL gl, string name, out uint buffer) => gl.CreateBuffer($"EBO: {name}", out buffer);
